...
then
	docker run -t -v "${PWD}:/libfwsi" ${DOCKERHUB_REPO}:${DOCKERHUB_TAG} sh -c "cd libfwsi && .travis/script_docker.sh";
...
then
	...
	then
		...
		cp AUTHORS COPYING NEWS README ${PWD}/osx-pkg/usr/share/doc/libfwsi;
