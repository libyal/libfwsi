/*
 * Library network_location_values type test program
 *
...
#include "../libfwsi/libfwsi_network_location_values.h"

uint8_t fwsi_test_network_location_values_data1[ 20 ] = {
	0x12, 0x00, 0x47, 0x00, 0x02, 0x45, 0x6e, 0x74, 0x69, 0x72, 0x65, 0x20, 0x4e, 0x65, 0x74, 0x77,
	0x6f, 0x72, 0x6b, 0x00 };

#if defined( __GNUC__ ) && !defined( LIBFWSI_DLL_IMPORT )

/* Tests the libfwsi_network_location_values_initialize function
 * Returns 1 if successful or 0 if not
 */
int fwsi_test_network_location_values_initialize(
     void )
{
	...
	/* Test regular cases
	 */
	result = libfwsi_network_location_values_initialize(
	...
	result = libfwsi_network_location_values_free(
	          &network_location_values,
	          &error );
	...
	network_location_values = (libfwsi_network_location_values_t *) 0x12345678UL;
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libfwsi_network_location_values_initialize with malloc failing
		 */
		...
		if( fwsi_test_malloc_attempts_before_fail != -1 )
		{
			...
			{
				libfwsi_network_location_values_free(
				 &network_location_values,
				 NULL );
			}
		}
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libfwsi_network_location_values_initialize with memset failing
		 */
...
/* Tests the libfwsi_network_location_values_free function
 * Returns 1 if successful or 0 if not
 */
int fwsi_test_network_location_values_free(
     void )
{
	libcerror_error_t *error = NULL;
	int result               = 0;
...
/* Tests the libfwsi_network_location_values_read_data function
 * Returns 1 if successful or 0 if not
 */
int fwsi_test_network_location_values_read_data(
     void )
{
	libcerror_error_t *error                                   = NULL;
	libfwsi_network_location_values_t *network_location_values = NULL;
	...
	result = libfwsi_network_location_values_read_data(
	          network_location_values,
	          fwsi_test_network_location_values_data1,
	          20,
...
#endif
{
	...
	FWSI_TEST_RUN(
	 ...
	 fwsi_test_network_location_values_initialize );

	FWSI_TEST_RUN(
	 "libfwsi_network_location_values_free",
	 fwsi_test_network_location_values_free );

	FWSI_TEST_RUN(
	 "libfwsi_network_location_values_read_data",
	 fwsi_test_network_location_values_read_data );
